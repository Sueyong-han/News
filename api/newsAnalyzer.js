  ...
  for (let hw of howList) {
    ...
    try {
      ...
      for (let item of items) {
        ...
        const prompt = `"${desc} 기사 날짜: ${date}" 이 내용과 인터넷을 보고 이 내용이 소설의 내용이거나 실제로 일어난것이 아닌 비유적인 표현이라면 XX만 출력하고 아니면, 범죄 종류와 날짜, 위치를 두서없이 '범죄 종류: 살인,절도,강간,교통사고,강도 중 하나, 또는 기타:(범죄 종류) | 범죄 날짜: ----년 --월 --일 | 범죄 위치: --시 --구 --동 ' 이런 식으로 나타내. 또한, 너가 나타낸 범죄 위치가 ${where}과 상관없다면 XX만 출력해. 그리고 `;
